...
export function SummaryEvidence({ item, transcripts, screens }: { item: SummaryItem; transcripts: TranscriptEvent[]; screens: ScreenEvent[] }) {
  ...
  return <div className="summary-evidence">
    ...
    {evidence.transcripts.length > 0 && <details className="summary-speech"><summary>根拠の発話を見る（{evidence.transcripts.length}件）</summary>{evidence.transcripts.map((transcript) => <blockquote key={transcript.id}><small>{time(transcript.startedAtMs)} · {transcript.speaker === "self" ? "あなた" : transcript.speaker === "remote" ? "参加者" : "話者不明"}</small><p>{transcript.text}</p></blockquote>)}</details>}
