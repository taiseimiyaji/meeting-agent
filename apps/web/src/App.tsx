...
function TranscriptPanel({ items, loading, focus, onScreen }: { items: TranscriptEvent[]; loading: boolean; focus?: string; onScreen: (id: string) => void }) {
  ...
  return <section className="panel"><div className="panel-title"><h2>Transcript</h2><span>{visible.length} events</span></div>{echoCount > 0 && <div className="echo-notice"><p>スピーカー音の回り込みと思われる重複 {echoCount}件を{showEchoes ? "表示中" : "まとめています"}。元の記録は保持しています。</p><label><input type="checkbox" checked={showEchoes} onChange={(event) => setShowEchoes(event.target.checked)}/>重複候補も表示</label></div>}{loading ? <Empty>読み込み中…</Empty> : !visible.length ? <Empty>Transcriptはまだありません。</Empty> : <div className="transcript-list">{visible.map((item) => <article id={`transcript-${item.id}`} className={focus === item.id ? "focused" : ""} key={`${item.id}-${item.revision}`}><time>{formatTime(item.startedAtMs)}</time><div><div className="speaker"><b>{item.possibleEchoOf ? "マイク（回り込み候補）" : item.attributedSpeaker ? item.attributedSpeaker.name : item.speaker === "self" ? "あなた" : item.speaker === "remote" ? "話者不明（システム音声）" : "話者不明"}</b>{item.attributedSpeaker && <span className="partial" title={item.attributedSpeaker.reason}>画面とAI照合</span>}{!item.isFinal && <span className="partial">文字起こし中…</span>}</div><p>{item.text}</p>{item.attributedSpeaker && <div className="refs">{item.attributedSpeaker.evidenceIds.filter((screenId) => screenId !== item.id).map((screenId) => <button key={screenId} onClick={() => onScreen(screenId)}>発話者の根拠画面</button>)}</div>}{item.screenRefs.length > 0 && <div className="refs">{item.screenRefs.map((ref) => <button key={`${ref.screenId}-${ref.relation}`} onClick={() => onScreen(ref.screenId)}>▧ {ref.screenId} <small>{Math.round((ref.confidence ?? 0) * 100)}%</small></button>)}</div>}</div></article>)}</div>}</section>;
}
