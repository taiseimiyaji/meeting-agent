...
@main struct Verification {
    static func main() async throws {
        ...
        if args.first == "tab-server", args.count == 2 { try await browserServer(root: URL(fileURLWithPath: args[1])); return }
        if args.first == "browser" { try await verifyBrowserCapture(); return }
        if args.first == "storage" { try await verifyLosslessStorage(); return }
        ...
        try await verifyLosslessStorage()
        try await verifyBrowserCapture()
